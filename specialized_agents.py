...
#     Includes methods to log reasoning steps during processing. # Added comment
#     """
...
#     def _add_next_best_action(self, action: str, priority: str, description: str, category: str, icon: str = '🔹'): # Add icon
#          """Add a next best action recommendation"""
#          # Ensure NBA list exists
...
#         mentioned_explicitly = bool(mentioned_locations or mentioned_merchants) # Flag if user mentioned specifics
...
#         # Prioritize transactions matching explicit mentions
...
#         # If no specifics mentioned OR no matches found, look for general keywords like "declined transaction"
#         # Also consider if the prompt just asks about "a transaction" or "recent activity"
...
#         # If still no relevant transactions and it's a general query, maybe show the last few?
...
#         # Analyze identified transactions (especially declined ones)
...
#                 )
#                 # Note: Actual explanation happens in response construction below

#             # Build response for declined transactions
...
#                 self._take_action( # Log action taken for this specific transaction
...
#                 # Add context-specific explanations and Next Best Actions (NBAs) based on decline reason
...
#                      is_international = any(loc in location for loc in ["japan", "germany", "spain", "france", "italy"]) # Example international locations
...
#                           # Check if a travel notice exists and if it covers the location
...
#                                )
#                           else:
#                                # Notice exists but transaction still declined? Might be another issue.
...
#                      else: # Domestic unusual activity
...
#             else: # Multiple declined transactions found
#                 self._take_action( # Log action taken for multiple transactions
...
#                 for tx in declined_transactions[:3]: # Show details for the first few
...
#         elif relevant_transactions: # No declines, but other relevant transactions found (e.g., user asked about a specific merchant)
...
#             for tx in relevant_transactions[:5]: # Show first 5 matches
...
#                  )
#             else:
#                  # Mix of approved/other statuses (e.g., pending)
#                  response += "\nLet me know if you need more details on any of these."


#         else: # No relevant transactions found at all
...
#             # Suggest alternatives based on the query
...
#         # Final packaging of the response
#         return {
#             "response": response.strip(), # Remove leading/trailing whitespace
#             "reasoning_log": self.reasoning_log,
#             "next_best_actions": self.reasoning_log.get("next_best_actions", []) # Ensure key exists
#         }
...
#         # More comprehensive list, consider adding common cities/countries
#         location_keywords = [
#             "japan", "tokyo", "osaka", # Japan
#             "germany", "berlin", "munich", # Germany
#             "spain", "barcelona", "madrid", # Spain
#             "france", "paris", "nice", # France
#             "italy", "rome", "milan", # Italy
#             "usa", "us", "united states", "new york", "los angeles", "chicago", # USA
#             "uk", "united kingdom", "london", # UK
#             # Add more as needed
#         ]

#         # Use word boundaries to avoid partial matches (e.g., "rome" in "chrome")
#         for location in location_keywords:
#             if re.search(r'\b' + re.escape(location) + r'\b', prompt_lower):
#                 # Normalize (e.g., map "usa", "us" to "USA")
...
#                 if normalized_location not in locations: # Avoid duplicates
#                      locations.append(normalized_location)

#         self._add_analysis_step(f"Extracted locations from prompt: {locations}")
#         return locations
...
#         # List of potential merchants (expand significantly for real-world use)
#         # This often requires named entity recognition (NER) in a real system
#         merchant_keywords = [
...
#              "whole foods",
#              # Add airline names, hotel chains, common online retailers etc.
#              "delta", "united airlines", "marriott", "hilton", "ebay"
#         ]

#         # Simple keyword matching with word boundaries
#         # A more robust solution would use NER or fuzzy matching
#         for merchant in merchant_keywords:
#              # Escape special characters in merchant name for regex
#              escaped_merchant = re.escape(merchant)
#              if re.search(r'\b' + escaped_merchant + r'\b', prompt_lower):
#                   if merchant not in merchants: # Avoid duplicates
#                       merchants.append(merchant)


#         self._add_analysis_step(f"Extracted merchants from prompt: {merchants}")
#         return merchants
...
#         # Determine what the user wants to do with travel notice
...
#         # Extract countries/locations mentioned
...
#                     # Pre-fill NBA? Or wait for user confirmation? Let's wait.
#                     self._add_next_best_action(
...
#                 # Check if mentioned countries are already included
...
#                      # User mentioned countries already in the notice, maybe they want to change dates?
...
#                       )
#                 else:
#                      # No specific countries mentioned in the update request
#                      self._add_next_best_action(
...
#                 # Simulate fixing the status in the data for subsequent checks
#                 self.travel_notice_data['status'] = 'Active' # Simulate fix
...
#         # --- Post-intent processing: Check for related transaction issues ---
#         self._add_analysis_step("Performing post-intent analysis: Checking for transaction/notice mismatches.")
#         mismatch_found = False
#         if active_notice and notice_details.get('status', '').lower() == 'active': # Only check if notice is confirmed active
...
#                     # Basic country extraction from location string
#                     known_countries = ['japan', 'germany', 'spain', 'usa', 'france', 'italy', 'uk'] # Add more
...
#                          # Check if the decline reason is likely related to location/travel
...
#                       "Travel Services", "🌍"
#                  )
#                  # Append to the existing response if appropriate
#                  if response and not response.endswith("?"): # Avoid adding if response ends with a question
...
#                  elif not response: # If no primary response was generated, make this the primary
...
#         # Look for keywords related to activation/fixing first
#         if re.search(r"\b(activate|fix|enable|reactivate|confirm it[']?s active)\b", prompt_lower):
#              # Distinguish from general "activate travel" which might mean 'create'
#              if "activate travel" in prompt_lower and not any(k in prompt_lower for k in ["fix", "enable", "confirm"]):
#                   pass # Let it fall through to check create/update
#              else:
#                   self._add_analysis_step("Detected keywords related to activating or fixing a notice.")
#                   return "activate_notice"


#         # Then check for creation keywords
#         # Use careful phrasing to avoid matching "update" keywords accidentally
#         # (e.g., "add a travel notice" vs "add countries to notice")
...
#              # Add a check: if "update" or "change" is also present, prefer "update"
...
#         # Then check for update keywords
...
#         # If none of the above, assume the user wants to check the status
...
#         # This uses the same logic as TransactionAnalysisAgent._extract_locations
#         # Consider consolidating if logic remains identical
#         countries = []
#         prompt_lower = user_prompt.lower()
#         country_keywords = [
#             "japan", "tokyo", "osaka",
...
#             "france", "paris", "nice",
#             "italy", "rome", "milan",
#             "usa", "us", "united states", "america", # Added america
#             "uk", "united kingdom", "london", "england", "scotland", # Added UK specifics
#             "canada", "mexico", "australia", # Add more common destinations
#         ]
...
#         # No separate analysis step log here as it's called internally by process()
#         return countries
...
#             return False, {} # No notice data or no countries listed

#         details = self.travel_notice_data.copy() # Work with a copy

#         # Check if travel dates are valid and current/future
#         try:
#             start_date_str = details.get('travel_start', '')
#             end_date_str = details.get('travel_end', '')
#             if not start_date_str or not end_date_str:
#                  # Missing dates, cannot determine activity based on time
#                  return bool(details.get('countries')), details # Active if countries exist, regardless of date

#             # Attempt to parse dates - adjust format if needed based on your data
#             # Example format: "Month Day, Year" (e.g., "July 15, 2024")
#             date_format = "%B %d, %Y"
#             start_date = datetime.datetime.strptime(start_date_str, date_format).date()
#             end_date = datetime.datetime.strptime(end_date_str, date_format).date()
#             today = datetime.date.today()

#             # Considered active if today is within the start/end dates (inclusive) or notice is for the future
#             is_active_time = start_date <= today <= end_date or start_date > today

#             # Also consider the explicit status field, if present
#             status = details.get('status', 'Unknown').lower()
#             is_active_status = "active" in status or "submitted" in status # Consider submitted as potentially active

#             # Combine time and status checks: Must be within date range AND have an active/submitted status
#             # If status indicates error, it's technically active but flawed.
#             is_active = is_active_time and is_active_status

#             return is_active, details

#         except ValueError:
#             # Date parsing failed - perhaps format mismatch or invalid data
#             # Fallback: Consider active if countries are listed, but log the error potential
...
#         # Determine what the user wants to do with their card
#         intent = self._determine_intent(user_prompt)
#         self._add_decision_factor("determined_intent", intent)
#         self._add_analysis_step(f"Determined user intent: {intent}")


#         # Get card information (might be simulated or fetched)
...
#         # Check for potential card-related issues inferred from data
...
#                 # Focus on the most critical issue if multiple exist
...
#                           "Transaction Analysis", "📉" # Note: Might route to another agent
#                       )
...
#                 )
#             else:
#                 # Simulate marking the card as lost
...
#                 card_info['status'] = 'reported lost' # Simulate update
...
#                 )
#                 self._add_next_best_action(
#                     "Offer Digital Card Access", "Medium", # Changed wording slightly
...
#             # Check if the card is nearing expiry
#             # expires_soon = self._check_expiry(card_info) # Assumes a helper method _check_expiry exists
...
#             else: # Card is blocked/lost - replacement is usually automatic in that flow
#                  response = f"Since {card_desc} is currently '{card_info.get('status')}', a replacement is typically part of resolving that status. We've already started that process if it was reported lost/stolen." # Referencing the report_lost_stolen flow
#                  response += " If it was blocked for another reason, let's resolve that first. Can I help with the reason it was blocked?"
#                  # Avoid redundant NBAs if handled in other intents

#             if card_info.get('status', '').lower() == "active" or expires_soon: # Only offer these if card isn't already blocked
#                 self._add_next_best_action(
#                     "Expedite Replacement Shipping", "Low", # Changed priority
...
#             # Add others if available (e.g., per transaction, credit limit)
#             if 'credit_limit' in limits:
#                  response += f"- Total Credit Limit: ${limits.get('credit_limit'):,}\n"
#                  response += f"- Available Credit: ${limits.get('available_credit'):,}" # Requires fetching live balance

#             # Check eligibility for limit increase (example)
#             eligible_for_increase = card_info.get('eligible_for_credit_increase', False) # Assumes this flag exists
...
#                 self._add_next_best_action(
#                     "Request Credit Limit Increase", "Medium", # Changed priority
...
#         else:  # General card inquiry (intent == "general_inquiry")
...
#             # Add other interesting static info if available, e.g., rewards program, contactless feature
#             # response += f" It's enrolled in our {card_info.get('rewards_program', 'standard rewards')} program."

#             if inferred_card_issues:
#                 response += "\n\nWhile checking, I did notice a potential issue based on recent activity:"
#                 issue = inferred_card_issues[0] # Focus on one primary issue for general inquiry
#                 response += f"\n- {issue}"
#                 response += "\n\nCan I help you look into this further?"
#                 # Add NBA relevant to the specific issue, similar to card_status intent
#                 if "lost" in issue:
...
#         # Prioritize lost/stolen due to security urgency
#         if re.search(r"\b(lost|stolen|missing|can'?t find my card|someone took my card)\b", prompt_lower):
#              # Avoid matching "lost" in other contexts if possible, but usually specific enough
#              self._add_analysis_step("Detected keywords related to lost or stolen card.")
#              return "report_lost_stolen"


#         # Check for replacement requests (damaged, expiring, general replacement)
#         # Differentiate from lost/stolen replacement flow
#         if re.search(r"\b(replace|replacement|new card|damaged|broken|expired|expiring soon)\b", prompt_lower):
#              # Check if it's explicitly NOT lost/stolen context
...
#                  return "replace_card"
#              # If lost/stolen is mentioned, let that intent take precedence


#         # Check for inquiries about limits
...
#         # In a real system, this would fetch data from a database/API
#         # For now, use data primarily from customer_data and transaction analysis
#         self._add_analysis_step("Fetching basic card information (simulated).")

#         card_info = {
#             "card_type": self.customer_data.get("card_type", "Credit Card"),
#             "last_four": self.customer_data.get("card_last_four", "7842"), # Get from customer data if available
...
#             # Add expiry date if available: "expiry_date": "MM/YY"
#         }

#         # Infer status from transactions if possible (most reliable is 'reported lost')
#         for tx in self.recent_transactions:
#             reason = tx.get("reason", "").lower()
...
#             # Could add checks for fraud blocks if reason codes exist
#             # elif "fraud block" in reason:
#             #     card_info["status"] = "blocked (fraud)"
#             #     break

#         # Could also check an explicit status field if provided in customer_data
#         # explicit_status = self.customer_data.get("card_status")
#         # if explicit_status and card_info["status"] == "active": # Don't override inferred lost status
#         #    card_info["status"] = explicit_status
...
#         issues = []
#         card_status = self._get_card_info().get('status', 'unknown').lower() # Get current status again

#         # Check 1: Is the card explicitly not active?
#         if card_status != "active":
#              issues.append(f"The card is currently marked as '{card_status}'.")


#         # Check 2: Are there recent declined transactions? (Focus if card *is* active)
#         if card_status == "active":
...
#         # Check 3: Mismatch between active status and 'reported lost' decline reason? (Indicates data inconsistency)
...
#         # Check 4: Travel notice issues affecting card usage?
#         # This might be better handled by TravelNoticeAgent, but CardServices can be aware
...
#              # Check if there were international declines potentially related
#             international_declines = []
#             for tx in self.recent_transactions:
#                  if tx.get('status', '').lower() == 'declined':
#                      location = tx.get('location', '').lower()
#                      is_international = any(loc in location for loc in ["japan", "germany", "spain", "france", "italy", "uk"]) # Example list
#                      if is_international:
...
#         # These would come from account/card product details
...
#              # Simulate available credit (needs live data)
#              # Assumes roughly 30% utilization for example
#              limits["available_credit"] = int(limits["credit_limit"] * 0.7)


#         return limits

# # --- GeneralInquiryAgent remains the same as provided previously ---
...
#         # Determine the type of general inquiry
...
#         # Gather relevant account information to enrich the response
...
#             # Optional: Add balance if easily available, but balance inquiry is separate intent
#             # response += f"- Current Average Balance: {account_summary.get('average_balance', 'N/A')}\n"

#             # Add highlights based on summary flags
...
#             # NOTE: Providing a *live* balance would require an API call here. Average is often static.

#             # Add context about recent transactions influencing balance
...
#         else:  # General help / Unclear inquiry
...
#             # Add more capabilities based on all agents

#             response += "\nHow can I help you specifically today?"

#             # Add personalized NBA based on account summary
#             if account_summary.get("has_declined_transactions"):
#                 self._add_next_best_action(
#                     "Discuss Recent Declines", "Medium", # More direct wording
#                     "Offer to look into the recent declined transactions.",
#                     "Transaction Analysis", "📉"
#                 )
#             elif account_summary.get("has_travel_notice_issue"):
#                 self._add_next_best_action(
#                     "Resolve Travel Notice Issue", "High", # More direct wording
...
#                    )
#             else:
#                 # Generic NBA if no specific issues flagged
#                 self._add_next_best_action(
...
#         # Contact preference keywords
#         if re.search(r"\b(contact|email|phone|text|sms|notification|preferences|how.*contact me)\b", prompt_lower):
#              # Avoid matching if the context is clearly about something else (e.g., "email confirmation of travel notice")
...
#         # If none of the above, or keywords like "help", "support", "question" are dominant
...
#         # Fallback if somehow missed (should ideally be covered by above)
...
#             # Flags based on analysis of other data sources
...
#             "card_status": "active" # Default, CardServicesAgent might provide more accurate status if called first
#         }

#         # Refine card status based on transaction reasons
#         for tx in self.recent_transactions:
#              reason = tx.get("reason", "").lower()
#              if "card reported lost" in reason or "stolen" in reason:
...
#     agent_class = agent_map.get(agent_name, GeneralInquiryAgent) # Default to GeneralInquiryAgent
#     print(f"[Factory] Creating instance of: {agent_class.__name__}") # Added print for debugging

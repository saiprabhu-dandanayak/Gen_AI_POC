...
def analyze_with_openai(transcript, customer_data, travel_notice_data, recent_transaction, model, openai_api_key, update_callback=None):
    sentiment_result = {}
    recommended_actions = []
    ...
    # Initialize RouterAgent with OpenAI API key
    ...
    router = RouterAgent(customer_data, travel_notice_data, recent_transaction if isinstance(recent_transaction, list) else [recent_transaction], openai_api_key, model)
    
    # Detailed routing process
    chain_of_thought += f"User query received: '{transcript}'\n"
    chain_of_thought += "Step 1: Sending query to OpenAI API for AI-based routing...\n"
    ...
    chain_of_thought += "\n=== Sentiment Analysis ===\n"
    chain_of_thought += "Analyzing sentiment using OpenAI API...\n"
    logger.info("Starting sentiment analysis")
    sentiment_messages = [
        {"role": "system", "content": sentiment_prompt.format(transcript=transcript)}
    ]
    sentiment_response, sentiment_error = make_openai_request(sentiment_messages, model, openai_api_key)
    logger.info(f"🚀 ~ sentiment_response: {sentiment_response}")
    if sentiment_error:
    ...
    chain_of_thought += "Generating next best actions using OpenAI API...\n"
    ...
    action_response, action_error = make_openai_request(action_messages, model, openai_api_key)
    if action_error:
    ...
    chain_of_thought += "Generating comprehensive reasoning narrative using OpenAI API...\n"
    ...
    reasoning_response, reasoning_error = make_openai_request(reasoning_messages, model, openai_api_key, max_tokens=2000)
    if reasoning_error:
    ...
    chain_of_thought = re.sub(r'[ \t]+', ' ', chain_of_thought)  # Replace multiple spaces/tabs with single space
    chain_of_thought = re.sub(r'\n{3,}', '\n\n', chain_of_thought)  # Reduce 3+ newlines to 2
    chain_of_thought = chain_of_thought.strip()  # Remove leading/trailing whitespace
    if update_callback:
        update_callback(chain_of_thought)
        st.session_state.chain_of_thought = chain_of_thought
...
def make_openai_request(messages, model, openai_api_key, temperature=0.7, max_tokens=1000):
    ...
    try:
        ...
        logger.debug(f"Sending OpenAI API request: {messages}")
...
if st.button("🔍 Run AI Analysis", use_container_width=True):
    if not st.session_state.api_key_set or not st.session_state.openai_api_key:
        ...
        with st.spinner("Starting live AI analysis..."):
            ...
            sentiment_result, recommended_actions, chain_of_thought = analyze_with_openai(
                transcript,
